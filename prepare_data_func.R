...
calculate_weight_of_evidence_discrete <- function(train_data, WoE_col, target_col) {
  ...
  df_final <- df_final %>%
    mutate(diff_prop_good = prop_good - lag(prop_good, default = first(prop_good)),
           ...
           IV = sum((prop_n_good - prop_n_bad) * WoE))
  df_final <- df_final %>%
    arrange(WoE) %>%
...
train_data$state_trnsfrmd <- ifelse(train_data$addr_state %in% state1, "state_list_1",
...
ggplot(data = int_rate_factor_woe, aes(x = int_rate_factor, WoE)) + 
  geom_line(aes(y = WoE, group = 1), color = 'blue') + 
  geom_point(aes(y = WoE, group = 1), color = 'blue')
train_data$int_rate_trnsfrmd <- ifelse(train_data$int_rate <= 9.548, 'int_rate<9.548',
...
distinct(train_data, int_rate_trnsfrmd)
...
ggplot(data = funded_amnt_factor_woe, aes(x = funded_amnt_factor, WoE)) + 
  geom_line(aes(y = WoE, group = 1), color = 'blue') + 
  geom_point(aes(y = WoE, group = 1), color = 'blue')
train_data$funded_amnt_factor_trnsfrmd <- train_data$funded_amnt_factor
...
train_data$mths_since_earliest_cr_line_trnsfrmd <- ifelse(train_data$mths_since_earliest_cr_line <= 140, 'mths_since_earliest_cr_line<=140',
...
train_data$total_acc_trnsfrmd <- ifelse(train_data$total_acc <= 10, 'total_acc<=10',
...
train_data$total_rev_hi_lim_factor <- cut(train_data$total_rev_hi_lim, 2000)
...
train_data$total_rev_hi_lim_trnsfrmd <- ifelse(train_data$total_rev_hi_lim <= 5000, 'total_rev_hi_lim<=5K',
                                                                           ...
                                                                           ifelse(train_data$total_rev_hi_lim <= 50000, 'total_rev_hi_lim:40K-50K',
                                                                                  ifelse(train_data$total_rev_hi_lim <= 100000, 'total_rev_hi_lim:50K-100L',
                                                                                         ifelse(train_data$total_rev_hi_lim > 100000, 'total_rev_hi_lim>100K',
                                                                                                NaN))))))))
distinct(train_data, total_rev_hi_lim_trnsfrmd)
...
train_data$installment_trnsfrmd <- ifelse(train_data$installment <= 30, 'installment<=30',
...
train_data$annual_inc_trnsfrmd <- ifelse(train_data$annual_inc <= 20000, 'annual_inc<=20K',
...
train_data$mths_since_last_delinq_trnsfrmd <- ifelse(train_data$mths_since_last_delinq <= 3, 'mths_since_last_delinq<=3',
...
train_data <- train_data %>%
  left_join(train_data_temp %>% select(V1, id, member_id, dti_factor))
...
train_data$dti_trnsfrmd <- ifelse(train_data$dti <= 35, paste0('dti:', train_data$dti_factor),
...
train_data$mths_since_last_record_trnsfrmd <- ifelse(train_data$mths_since_last_record <= 2, 'mths_since_last_record<=2',
...
train_data_trnsfrmd <- train_data %>%
  select(ends_with(c('V1', 'id', 'trnsfrmd')))
