...
# loan_data <- fread("loan_data_2007_2014.csv", nrows = 480000)
...
# loan_data <- loan_data %>%
#   mutate(emp_length_int = str_replace(emp_length_int, " years", "")) %>%
#   mutate(emp_length_int = str_replace(emp_length_int, " year", "")) %>%
#   mutate(emp_length_int = str_replace(emp_length_int, "10+", "10")) %>%
#   mutate(emp_length_int = as.double(emp_length_int)) %>%
#   mutate(emp_length_int = ifelse(is.finite(emp_length_int), 0, emp_length_int))

distinct(loan_data, emp_length_int)

# convert term_int column to integers
# loan_data$term_int <- map_chr(loan_data$term, numextract) %>%
#   as.double()
loan_data <- loan_data %>%
...
# loan_data <- loan_data %>%
#   mutate(mths_since_earliest_cr_line = ifelse(mths_since_earliest_cr_line < 0, 
#                                                max(mths_since_earliest_cr_line, na.rm = TRUE), 
...
train_index <- createDataPartition(loan_data$good_bad,p=0.8,list=FALSE)
train_data <- loan_data[train_index,]
test_data <- loan_data[-train_index,]

source('prepare_train_data.R')
source('prepare_test_data.R')
rm(list= ls()[!(ls() %in% c('loan_data','train_data', 'train_data_trnsfrmd', 'test_data', 'test_data_trnsfrmd', 'IV_df'))])
